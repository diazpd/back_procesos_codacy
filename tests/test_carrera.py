...
class TestCarrera(TestCase):
    ...
    def test_lista_carreras(self):
        ...
        total_carreras_inicio = len(json.loads(solicitud_consultar_carreras_antes.get_data()))
        nueva_carrera = {
            "nombre": "GP de Miami",
            "competidores": [
                {
        ...
        solicitud_consultar_carreras_despues = self.client.get(endpoint, headers=headers)
        total_carreras = len(json.loads(solicitud_consultar_carreras_despues.get_data()))

     

        self.assertEqual(solicitud_consultar_carreras_despues.status_code, 200)
        self.assertGreater(total_carreras, total_carreras_inicio)
